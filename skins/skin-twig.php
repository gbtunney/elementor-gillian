...
namespace ElementorHelloWorld\Skins;
...
use ElementorPro\Modules\Posts\Skins\Skin_Base as Skin_Base;
//use Timber;
//use Timber\Post;
use Timber;
if ( ! defined( 'ABSPATH' ) ) {
...
class Skin_Twig extends Skin_Base {
    ...
    public function register_controls( Widget_Base $widget ) {
        ...
        $this->register_template_control();
    ...
    public function register_template_control(){
        $this->add_control(
            'twig_template',
            [
                'label' => __( 'Twig Template', 'plugin-domain' ),
                'type' => \Elementor\Controls_Manager::CODE,
                'default' => __( '<h1>{{ post.id }}</h1>', 'plugin-domain' ),
                'placeholder' => __( 'twig template!', 'plugin-domain' ),
            ]
        );
    }
    ...
    protected function render_template() {
        ...
        $setting_key = $this->get_control_id( 'twig_template' );
       $timberPost = new Timber\Post(get_the_ID());
       $context = array_merge(array(),array("post" => $timberPost, "test" => "green"));
       $content =  $settings[$setting_key];//'<h1>{{ post.id }}I AM A TEMPLATE!{{ post.title }}</h1>';
        /*  if (isset($timberPost->test_image) && strlen($timberPost->test_image)) {
              $timberPost->test_image = new Timber\Image($timberPost->test_image);
          }*/

       // echo do_shortcode('[elementor-template id="2500"]');
           echo   Timber\Timber::compile_string(stripslashes($content), $context);
//rint_r( $timberPost);
        ?>
        <?php
    }
